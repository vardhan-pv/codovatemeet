...
export function useMeetingRecorder(options: MeetingRecorderOptions = {}) {
  ...
  const startRecording = useCallback(async (mode: RecordingMode = 'full_meeting', quality: RecordingQuality = '720p') => {
    try {
      chunksRef.current = []
      setActiveMode(mode)
      ...
      if (mode === 'audio_only') {
        ...
        // Full meeting mode: capture camera + mic (or screen + mic)
  ...
  const downloadRecording = useCallback((filename?: string) => {
    ...
    const ext = activeMode === 'audio_only' ? 'webm' : 'webm'
