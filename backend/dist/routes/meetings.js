...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`SELECT m.*, u.name as host_name, mh.duration as actual_duration
       FROM meetings m 
       JOIN users u ON m.host_id = u.id 
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        await (0, db_1.query)('INSERT INTO meetings (id, meeting_code, host_id, room_name, scheduled_at, type, duration_minutes) VALUES ($1, $2, $3, $4, $5, $6, $7)', [meetingId, meetingCode, userId, roomName, new Date(scheduledAt), type, Number(durationMinutes)]);
...
router.post('/save-summary', async (req, res) => {
    try {
        ...
        const summaryId = crypto_1.default.randomUUID();
        await (0, db_1.query)(`INSERT INTO meeting_summaries (id, meeting_id, summary_text, key_points, action_items, decisions, follow_ups, provider)
...
router.get('/:code/summary', async (req, res) => {
    try {
        ...
        const result = await (0, db_1.query)(`SELECT ms.* FROM meeting_summaries ms
